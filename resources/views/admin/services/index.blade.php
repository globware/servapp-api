        ...
        <div class="bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <select class="block pl-3 pr-10 py-2.5 text-sm border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 rounded-xl bg-gray-50 transition-all duration-200 min-w-35">
                            ...
                            <tr class="hover:bg-gray-50/50 transition-colors duration-200">
                                <td class="px-8 py-5 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="shrink-0 h-12 w-12 bg-indigo-50 rounded-2xl flex items-center justify-center text-indigo-600 font-black text-xl border border-indigo-100">
                                            {{ substr($service->name ?? 'S', 0, 1) }}
                                        </div>
                                        <div class="ml-4">
                                            <div class="text-sm font-bold text-slate-900 truncate max-w-50">
                                ...
                                <td class="px-8 py-5 whitespace-nowrap">
                                    ...
                                    <div class="text-[10px] text-slate-400 mt-2 font-bold tracking-tight">
                                        {{ $service->requests_count ?? 0 }} Engagement requests
                                    </div>
                                </td>
                                <td class="px-8 py-5 whitespace-nowrap">
                                    <div class="flex flex-col">
                                        <span class="text-sm font-bold text-slate-700">{{ $service->state->name ?? 'N/A' }}</span>
                                        <span class="text-[10px] text-slate-400 font-medium">{{ $service->country->name ?? 'N/A' }}</span>
                                    </div>
                                </td>
                                <td class="px-8 py-5 whitespace-nowrap">
                                    @if($service->verified)
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider bg-emerald-50 text-emerald-600 border border-emerald-100">
                                    ...
                                    @else
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider bg-amber-50 text-amber-600 border border-amber-100">
                                            Pending
                                        </span>
                                    @endif
                                </td>
                                <td class="px-8 py-5 whitespace-nowrap text-right text-sm font-medium" id="verify-cell-{{ $service->id }}">
                                    <div class="flex items-center justify-end space-x-2">
                                        <span id="verify-span-{{ $service->id }}">
                                            @if(!$service->verified)
                                                <button 
                                                    ...
                                                    class="inline-flex items-center justify-center p-2 text-emerald-600 hover:bg-emerald-50 rounded-xl transition-all duration-200 group"
                                        ...
                                        <a href="{{ route('admin.services.show', $service->id) }}" 
                                            class="inline-flex items-center justify-center p-2 text-indigo-600 hover:bg-indigo-50 rounded-xl transition-all duration-200"
                                            title="View Details">
                                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <tr>
                                <td colspan="5" class="px-8 py-12 text-center text-sm text-gray-500">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-slate-500 font-bold">No services found.</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
